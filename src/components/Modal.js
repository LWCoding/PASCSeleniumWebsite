...
const Modal = ({ nameInput, descInput, setIsOpen, allowEnrichmentChange }) => {
  ...
  function handleEnrichmentChange(event) {
    fetch("http://localhost:3000/find-enrichment?name=" + event.value).then((res) => {
      return res.json();
    }).then((json) => {
  ...
  return (
    <div>
        ...
        <div className="modal">
            <img src={closeBtn} alt="Close button" className="modal-close-btn" onClick={ () => setIsOpen(false) } />
            <h2 className="modal-name">{name}</h2>
            <p className="modal-description">{desc}</p>
            {allowEnrichmentChange && 
            <Select className="modal-select" 
                    onChange={(e) => handleEnrichmentChange(e)} options={enrich.map((enr) => (
                                {value: enr.name, label: enr.name}
                              ))}>
            </Select>}
        </div>
    </div>
